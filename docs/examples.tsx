...
export function LayoutAndRecords() {
  ...
  return <div className="catalog-demo-stack">
    ...
    <Example title="Photo and avatar upload" names="UiPhotoUpload · UiAvatarUpload"><div className="catalog-field-grid"><div><strong>Photo field</strong><UiPhotoUpload label="Record photo" onChange={file=>{setPhoto(file);if(!file)setPhotoSrc(undefined)}} src={photoSrc} value={photo}/></div><div><strong>Avatar crop</strong><UiAvatarUpload onChange={setCroppedAvatar} src={taskIllustration}/>{croppedAvatar && <p>Cropped PNG ready: {croppedAvatar.name}</p>}</div></div></Example>
