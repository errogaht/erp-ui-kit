...
export function LayoutAndRecords() {
  ...
  return <div className="catalog-demo-stack">
    ...
    <Example title="Photo and avatar upload" names="UiPhotoUpload · UiAvatarUpload"><div className="catalog-field-grid"><UiStack gap="relaxed"><div><strong>Photo with preview</strong><UiPhotoUpload label="Record photo" onChange={file=>{setPhoto(file);if(!file)setPhotoSrc(undefined)}} src={photoSrc} value={photo}/></div><div><strong>Empty photo field</strong><UiPhotoUpload label="Additional photo" onChange={setExtraPhoto} value={extraPhoto}/></div></UiStack><div><strong>Avatar crop</strong><UiAvatarUpload onChange={setCroppedAvatar} src={portraitIllustration}/>{croppedAvatar && <p>Cropped PNG ready: {croppedAvatar.name}</p>}</div></div></Example>
...
export function StatesAndNavigation() {
  ...
  return <div className="catalog-demo-stack">
    ...
    <Example title="Loading, empty state and help" names="UiSkeleton · UiEmpty · UiHelp"><UiGrid><UiCell mobile={12} tablet={6}><UiSkeleton lines={3}/></UiCell><UiCell mobile={12} tablet={6}><UiStack gap="normal"><UiEmpty>No records match this filter.</UiEmpty><UiHelp label="How filters work">Filters apply only to the current list.</UiHelp></UiStack></UiCell></UiGrid></Example>
    ...
    <Example title="Sidebar and horizontal navigation" names="UiSidebarNav · UiTopNav"><UiTopNav activeId="records" items={[{id:'overview',label:'Overview',href:'#information',icon:'grid'},{id:'records',label:'Records',href:'#admin-patterns',icon:'table',badge:'8'},{id:'tasks',label:'Tasks',href:'#tasks',icon:'check2-square'}]}/><div className="catalog-nav-preview"><UiSidebarNav activeId="records" items={[{id:'overview',label:'Overview',href:'#information',icon:'grid'},{id:'records',label:'Records',href:'#admin-patterns',icon:'table',badge:'8'},{id:'tasks',label:'Tasks',href:'#tasks',icon:'check2-square'}]} title="Operations"/><UiPanel title="Records overview"><UiFacts items={[{label:'Open',value:'5 records'},{label:'In review',value:'2 records'},{label:'Updated',value:'Today'}]}/></UiPanel></div></Example>
  </div>
}
