...
export function UiCrudScreen({ title, description, rows, columns, fields, pageSize = 8, bulkActions = [], importAccept = '.json', validate, onCreate, onUpdate, onDelete, onImport, onExport, className = '' }: {
  ...
  // Both table and responsive cards use the same selected page, not all filtered rows.
  const selectPage = (checked: boolean) => setSelected(current => { const next = new Set(current); visible.forEach(row => checked ? next.add(row.id) : next.delete(row.id)); return next })
  return <section aria-label={title} className={`ui-kit-crud ${className}`.trim()}>
    ...
    <div className="ui-kit-crud__toolbar"><label><UiBootstrapIcon name="search" /><UiInput aria-label="Search records" onChange={event => setQuery(event.target.value)} placeholder="Search records" type="search" value={query} /></label><span>{filtered.length} records</span><label className="ui-kit-crud__mobile-select"><input aria-label="Select all rows on this page" checked={visible.length > 0 && visible.every(row => selected.has(row.id))} onChange={event => selectPage(event.target.checked)} type="checkbox" /> Select page</label><label className="ui-kit-crud__mobile-sort">Sort by<UiSelect aria-label="Sort records by" onChange={event => { setSortKey(event.target.value); setSortDescending(false) }} value={sortKey}>{columns.map(column => <option key={column.key} value={column.key}>{column.label}</option>)}</UiSelect></label><button aria-label={sortDescending ? "Sort ascending" : "Sort descending"} className="ui-kit-crud__mobile-direction" onClick={() => setSortDescending(value => !value)} type="button"><UiBootstrapIcon name={sortDescending ? "sort-up" : "sort-down"} /></button><div>{onImport && <><input accept={importAccept} aria-label="Import records file" hidden onChange={event => { const file = event.target.files?.[0]; if (file) onImport(file); event.target.value = '' }} ref={importInput} type="file" /><UiButton onClick={() => importInput.current?.click()} type="button"><UiBootstrapIcon name="upload" /> Import</UiButton></>}{onExport && <UiButton onClick={() => onExport(selectedRows.length ? selectedRows : filtered)} type="button"><UiBootstrapIcon name="download" /> Export</UiButton>}</div></div>
    ...
    <div className="ui-kit-crud__table"><UiTable><thead><tr><th><input aria-label="Select all rows on this page" checked={visible.length > 0 && visible.every(row => selected.has(row.id))} onChange={event => selectPage(event.target.checked)} type="checkbox" /></th>{columns.map(column => <th aria-sort={sortKey === column.key ? sortDescending ? 'descending' : 'ascending' : undefined} key={column.key}><button aria-label={`Sort by ${column.label}`} onClick={() => { if (sortKey === column.key) setSortDescending(value => !value); else { setSortKey(column.key); setSortDescending(false) } }} type="button">{column.label} {sortKey === column.key && <UiBootstrapIcon name={sortDescending ? 'arrow-down' : 'arrow-up'} />}</button></th>)}<th>Actions</th></tr></thead><tbody>{visible.map(row => <tr key={row.id}><td className="ui-kit-crud__select-cell"><input aria-label={`Select ${row.id}`} checked={selected.has(row.id)} onChange={() => toggle(row.id)} type="checkbox" /></td>{columns.map(column => <td data-label={column.label} key={column.key}><span className="ui-kit-crud__mobile-label">{column.label}</span>{column.render ? column.render(row[column.key], row) : String(row[column.key] ?? '—')}</td>)}<td className="ui-kit-crud__actions-cell"><div className="ui-kit-crud__row-actions">{onUpdate && <button aria-label={`Edit ${row.id}`} onClick={() => openEdit(row)} type="button"><UiBootstrapIcon name="pencil" /> Edit</button>}{onDelete && <button aria-label={`Delete ${row.id}`} onClick={() => setDeleting(row.id)} type="button"><UiBootstrapIcon name="trash" /> Delete</button>}</div></td></tr>)}{visible.length === 0 && <tr><td className="ui-kit-crud__empty" colSpan={columns.length + 2}>No records match the search.</td></tr>}</tbody></UiTable></div>
