...
`rows` is controlled. The host loads records and updates them after successful writes. The screen handles local search, sort, page selection, edit drafts and native required-field validation. `validate` can keep the dialog open with a record-specific error. `bulkActions` receives selected rows; deletion asks for confirmation. `onImport` gets a `File` and `onExport` receives either selected rows or the filtered list. The host decides file format, parsing, permissions and API calls; set `importAccept` to match its accepted format. The demo implements a local JSON round trip.

`UiPhotoUpload` supplies a chosen image `File` and previews it. `UiAvatarUpload` produces a circular 256px PNG after zoom and position adjustment. Both leave upload and storage to the host. `UiSidebarNav` and `UiTopNav` receive link items and a controlled `activeId`.
